...
class Questions(BaseModel):
    ...
    options: list[str] = Field(
        description="Exactly 4 multiple choice options, labeld A, B, C, D."
    )
    correct_naswer: str = Field(
...
class Quiz(BaseModel):
    ...
    questions: list[Questions] = Field(description="List of the quiz questions")
...
def generate_quiz(
    ...
    prompt = f"""
    Based on the following information, create a {difficulty} quiz, about {topic} with
    {num_questions} using the following research information.
    
    <RESEARCH_INFORMATION>
    {research_text}
    </RESEARCH_INFORMATION>
    
    Make sure to use the research information to create the most accuracte questions.
    """
