    ...
    return (
        <View style={styles.listFooter}>
            <View style={styles.listFooterAvatar}>
                <Image source={require("../assets/images/react-logo.png")} style={styles.avatarSmall} />
            </View>
            <View>
...
export default function Modal() {
    ...
    const canPost = threads.every((thread) =>
        thread.text.trim().length > 0 || thread.imageUris.length > 0);
    ...
    const pickImage = async (id: string) => {
        let { accessPrivileges } = await ImagePicker.requestMediaLibraryPermissionsAsync();
        console.log("status : ", accessPrivileges);
        if (accessPrivileges === "none") {
            Alert.alert("Photo permission is not granted !",
                "Please grant photo",
                [
                    {
        ...
        if (!result.canceled) {
            setThreads((prevThreads) => prevThreads.map((thread) => thread.id === id ? {
                ...thread, imageUris: thread.imageUris.concat(result.assets?.map(asset => asset.uri) ?? [])
            } : thread))
        }
    };
    ...
    const takePhoto = async (id: string) => {
        ...
        if (granted && result.assets?.[0].uri) {
        ...
        <View style={styles.threadContainer}>
            ...
            <View style={styles.contentContainer}>
                <View style={styles.userInfoContainer}>
                    <Text style={styles.username}>lotto0</Text>
                    {index > 0 && (
                        <TouchableOpacity
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <FlatList data={threads}
                ...
                style={styles.list}
                contentContainerStyle={{ paddingBottom: 10, backgroundColor: "#ddd" }}
                keyboardShouldPersistTaps="handled"
                ListFooterComponent={<ListFooter canAddThread={canAddThread} addThread={() => {
                    if (canAddThread) {
            ...
            <View style={[styles.footer, { paddingBottom: insets.bottom + 10 }]}>
                ...
                <Pressable style={[styles.postButton, !canPost && styles.postButtonDisabled]} disabled={!canPost} onPress={handlePost}>
